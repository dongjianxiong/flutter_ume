...
class PageInfoHelper {
  ...
  void _selectionInit() {
    ...
    void findAllRenderObject(RenderObject object) {
      ...
      for (int i = 0; i < children.length; i++) {
        ...
        if (c.style == DiagnosticsTreeStyle.offstage || c.value is! RenderBox) continue;
  ...
  Future<String?> getCode() async {
    ...
    String? scriptId = await codeDisplayService.getScriptIdWithFileName(targetFileName);
    if (scriptId == null) return null;
    String? sourceCode = await codeDisplayService.getSourceCodeWithScriptId(scriptId);
    return sourceCode;
  }
  ...
  Future<String?> getCodeByFileName(String fileName) async {
    CodeDisplayService codeDisplayService = CodeDisplayService();
    String? sourceCode;
    String? scriptId = await codeDisplayService.getScriptIdWithFileName(fileName);
    if (scriptId != null) {
  ...
  Future<Map<String?, String>> getCodeListByKeyword(String keyword) async {
    ...
    if (scriptIds.isNotEmpty) {
      for (final entry in scriptIds.entries) {
        final code = await codeDisplayService.getSourceCodeWithScriptId(entry.key!);
