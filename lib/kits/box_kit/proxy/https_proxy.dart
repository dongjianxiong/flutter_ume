import 'dart:convert';

import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter/src/painting/image_provider.dart';
import 'package:flutter/src/widgets/framework.dart';
import 'package:flutter_ume/core/pluggable.dart';
import 'package:flutter_ume/flutter_ume.dart';

const httpsProxyIconData =
    r'';
final httpsProxyIconBytes = base64Decode(httpsProxyIconData);

class HttpsProxyPlugin implements Pluggable {
  final Function(String event) onFinish;

  HttpsProxyPlugin(this.onFinish);

  @override
  Widget? buildWidget(BuildContext? context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("设置代理"),
      ),
      body: InputForm(
        onFinish: (String event) {
          this.onFinish.call(event);
        },
      ),
    );
  }

  @override
  String get displayName => '设置网络代理';

  @override
  ImageProvider<Object> get iconImageProvider => MemoryImage(httpsProxyIconBytes);

  @override
  String get name => 'HttpsProxyPlugin'; //

  @override
  void onTrigger() {
    print('HzBoxKit: ${this.runtimeType} ======onTrigger=====');
  }
}

class InputForm extends StatefulWidget {
  final Function(String event) onFinish;
  const InputForm({required this.onFinish});

  @override
  _InputFormState createState() => _InputFormState();
}

class _InputFormState extends State<InputForm> {
  TextEditingController _controller = TextEditingController();

  @override
  void initState() {
    super.initState();
    // 读取之前保存的内容并显示在输入框中
    _controller.text = HzBoxSharedPref.getValueForKey('HzBoxHttpsProxy') ?? "";
  }

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.all(20.0),
      child: Column(
        children: [
          TextField(
            controller: _controller,
            decoration: InputDecoration(
              hintText: '输入IP地址及端口号(192.168.184.35:8888)',
            ),
          ),
          SizedBox(height: 20.0),
          ElevatedButton(
            onPressed: () {
              HzBoxSharedPref.saveValueForKey(_controller.text, 'HzBoxHttpsProxy');
              widget.onFinish.call(_controller.text);
            },
            child: Text('保存'),
          ),
        ],
      ),
    );
  }
}
