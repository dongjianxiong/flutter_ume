...
class _WidgetInfoInspectorState extends State<WidgetInfoInspector> with WidgetsBindingObserver {
  ...
  final window = PlatformDispatcher
      .instance.implicitView!; //bindingAmbiguate(WidgetsBinding.instance)!.window;
  ...
  void _handlePanEnd(DragEndDetails details) {
    final Rect bounds =
        (Offset.zero & (window.physicalSize / window.devicePixelRatio)).deflate(1.0);
...
class _DebugPaintButtonState extends State<_DebugPaintButton> {
  ...
  void _showAllSize() async {
    ...
    setState(() {
      ...
      bindingAmbiguate(RendererBinding.instance)?.renderViews.last.visitChildren(visitor);
    });
  }
