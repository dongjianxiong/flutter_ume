...
extension Utils on Color {
  ...
  /// 001232ac
  String get hexRGB => value.toRadixString(16).padLeft(8, '0').replaceRange(0, 2, '');

  Color withSaturation(double value) => HSLColor.fromAHSL(opacity, hue, value, lightness).toColor();
  ...
  List<Color> getShades(int stepCount, {bool skipFirst = true}) => List.generate(
        ...
        (index) {
          return hsl
              .withLightness(
                  1 - ((index + (skipFirst ? 1 : 0)) / (stepCount - (skipFirst ? -1 : 1))))
              .toColor();
        },
      );
}
...
ui.Color getPixelColor(img.Image image, Offset offset) {
  ...
  return (offset.dx >= 0 && offset.dy >= 0 && offset.dx < image.width && offset.dy < image.height)
